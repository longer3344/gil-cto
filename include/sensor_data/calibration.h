...
namespace liso {
	...
	struct SegmentCalibParam {
		SegmentCalibParam()
			...
			time_offset(0), starttime(0), endtime(0){}
	...
	struct CalibOptions {
		...
		CalibOptions(const YAML::Node& node) 
		{
			...
			std::cout << YELLOW << "time_offset_padding set as: " << time_offset_padding << RESET << std::endl;
		}
	...
	public:
		...
		{
			assert(segment_id < segment_param.size() && "[UpdateGravity] segment_id too big");
			segment_param.at(segment_id).gravity = gravity_in; // 重力矢量

			gravity_in = (gravity_in / GRAVITY_NORM).eval(); // 归一化
			double cr = std::sqrt(gravity_in[0] * gravity_in[0] + gravity_in[2] * gravity_in[2]);
		...
		void UpdateGravity() 
		{
			for (size_t i = 0; i < segment_param.size(); i++)
			{
				Eigen::Map<const Eigen::Matrix<double, 2, 1>> g_param(segment_param.at(i).g_refine.data());
				segment_param.at(i).gravity = gravity_factor::refined_gravity<double>(g_param);
			}
		}
		...
		void showStates() const {
			...
			for (size_t i = 0; i < segment_param.size(); i++) {
				...
				friendly_output(segment_param[i].acce_bias, 3, "accel bias", 10);
				friendly_output(segment_param[i].gyro_bias, 3, "gyros bias", 10);
			}
		...
		void showIMUBias(FILE *pFile = NULL) const {

			FILE *Fout = pFile ? pFile : stdout;
			for (size_t i = 0; i < segment_param.size(); i++)
			{
				fprintf(Fout, "%8zd %10.3lf %10.3lf %13.10lf %13.10lf %13.10lf %13.10lf %13.10lf %13.10lf\n", i,
					segment_param[i].starttime, segment_param[i].endtime,
					segment_param[i].acce_bias(0), segment_param[i].acce_bias(1), segment_param[i].acce_bias(2),
					segment_param[i].gyro_bias(0)*R2D, segment_param[i].gyro_bias(1)*R2D, segment_param[i].gyro_bias(2)*R2D);
			}
	...
	public:
		LiDAROdomParam lo_param;      ///< 激光里程计配置参数
		CalibWeights calib_weights;   ///< 优化权重
		CalibOptions calib_option;    ///< 优化选项

		Eigen::Vector3d p_LinI;		///< 激光雷达杆臂
		SO3d so3_LtoI;				///< 激光雷达安置角
		Eigen::Quaterniond q_LtoI;    ///< 激光雷达安置角
		SE3d se3_LtoI;                ///< 激光雷达外参
		Eigen::Vector3d p_LinI_prior; ///< 激光雷达杆臂先验值

		double	traj_starttime;     ///< 轨迹开始时间
		double	traj_endtime;       ///< 轨迹结束时间
		double	segment_interval;   ///< 轨迹分段时间间隔
		int		segment_number;     ///< 轨迹分段数
		std::vector<SegmentCalibParam> segment_param;		///< 每段的标定参数, 主要是惯导的零偏
