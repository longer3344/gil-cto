...
inline void PublishIMUData(std::shared_ptr<Trajectory> trajectory,
  ...
  for (auto const &v : imu_data) {
    ...
    auto const param = trajectory->GetTrajParam(v.timestamp); // 惯导零偏
    Eigen::Vector3d w_b = trajectory->rotVelBody(v.timestamp) + param->gyro_bias;
    Eigen::Vector3d a_w = trajectory->transAccelWorld(v.timestamp);
    SE3d pose = trajectory->pose(v.timestamp);
    Eigen::Vector3d a_b = pose.so3().inverse() * (a_w + param->gravity) + param->acce_bias;
